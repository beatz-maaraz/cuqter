...
class _AnimatedNotificationBellState extends State<AnimatedNotificationBell>
  ...
  bool _isNewer(dynamic ts, DateTime? lastSeen) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, userSnapshot) {
        ...
        if (userSnapshot.hasData && userSnapshot.data!.exists) {
          ...
          if (userData != null && userData['lastNotificationSeen'] != null) {
            lastSeen = _parseTimestamp(userData['lastNotificationSeen']);
          }
        }
              ...
              builder: (context, freqSnapshot) {
                ...
                if (notifSnapshot.hasData) {
                  for (var doc in notifSnapshot.data!.docs) {
                    ...
                    if (!isRead && _isNewer(data['timestamp'], lastSeen)) {
                      hasUnread = true;
                      break;
                    }
                  }
                ...
                if (!hasUnread && freqSnapshot.hasData) {
                  for (var doc in freqSnapshot.data!.docs) {
                    final data = doc.data() as Map<String, dynamic>;
                    if (_isNewer(data['timestamp'], lastSeen)) {
                      hasUnread = true;
                      break;
                    }
                  }
